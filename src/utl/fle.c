...
bool vb_fle_alloc_path_buffer(vb_fle_t *const file, vb_mem_t *const mem) {
	...
	return (bool)(file->buf != NULL);
}

bool vb_fle_binary_path(vb_fle_t *const file) {
	assert(file);
	bool ret_val = false;
	if (file->err == NULL) {
		if (file->buf) {
			if (file->buf->len > 1) {
				...
				if ((int64_t)buf_len == file->buf->len) {
					...
					if (path_len >= 0) {
						...
						file->err = vb_err_new(VB_ERR_FLE_CANT_DETERMINE_PATH, 0, "can't determine path of executable", NULL);
					}
				} else {
					file->err = vb_err_new(VB_ERR_FLE_OVERFLOW, 1, "buffer size overflow", NULL);
				}
			} else {
				file->err = vb_err_new(VB_ERR_FLE_UNDERFLOW, 1, "buffer size underflow", NULL);
			}
		} else {
			file->err = vb_err_new(VB_ERR_FLE_BUFFER_MISSING, 1, "buffer missing", NULL);
		}
	}
	return ret_val;
}
...
bool vb_fle_mkd(vb_fle_t *const file, const char *const path) {
	...
	if (file->err == NULL) {
		...
		if (!mkdir(path, 0777)) {
			...
			file->err = vb_err_new(VB_ERR_FLE_CANT_CREATE, 0, "can't create directory", NULL);
		}
	}
	return ret_val;
}
	...
	if (file->err == NULL) {
		...
		if (fd >= 0) {
			if (!close(fd)) {
				...
				file->err = vb_err_new(VB_ERR_FLE_CANT_CLOSE, 0, "can't close file", NULL);
			}
		} else {
			file->err = vb_err_new(VB_ERR_FLE_CANT_CREATE, 0, "can't create file", NULL);
		}
	}
	return ret_val;
}
	...
	if (file->err == NULL) {
		...
		if (!unlink(path)) {
			...
			if (!rmdir(path)) {
				...
				file->err = vb_err_new(VB_ERR_FLE_CANT_DELETE, 3, "can't delete directory", NULL);
			} else {
				file->err = vb_err_new(VB_ERR_FLE_CANT_DELETE, 2, "can't delete directory", NULL);
			}
		} else {
			file->err = vb_err_new(VB_ERR_FLE_CANT_DELETE, 1, "can't delete file", NULL);
		}
	}
	return ret_val;
}
		...
		if (file->buf) {
			if (file->buf->len > 1) {
				...
				if ((size_t)base_len == base_name_len) {
					...
					file->err = vb_err_new(VB_ERR_FLE_OVERFLOW, 2, "base name overflow", NULL);
				}
			} else {
				file->err = vb_err_new(VB_ERR_FLE_UNDERFLOW, 2, "buffer size underflow", NULL);
			}
		} else {
			file->err = vb_err_new(VB_ERR_FLE_BUFFER_MISSING, 2, "buffer missing", NULL);
		}
	}
	return ret_val;
}
...
bool vb_fle_stats(vb_fle_t *const file, const char *const path) {
	...
	if (file->err == NULL) {
		...
		if (!stat(path, &info)) {
			...
			file->type = VB_FLE_OTHER;
			file->exists = false;
			file->err = vb_err_new(VB_ERR_FLE_CANT_READ, 0, "can't read file", NULL);
		}
	}
	return ret_val;
}
